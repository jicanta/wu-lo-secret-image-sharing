...
void args_print_usage(const char *progname)
{
    fprintf(stderr,
            "Usage:\n"
            "  Distribute: %s -d -secret <image.bmp> -k <num> [-n <num>] [-dir <dir>]\n"
            "  Recover:    %s -r -secret <image.bmp> -k <num> [-n <num>] [-dir <dir>]\n"
            "\n"
            ...
            "  -dir <dir>       Directory containing carrier images (default: current dir).\n"
            "\n"
            "Examples:\n"
...
int args_parse(int argc, char *argv[], Args *out)
{
    ...
    /* ---- 4 & 5. Optional: -n and/or -dir (in any order) ---- */
    while (i < argc) {
