...
static int lagrange_eval_at_zero(const int* vals, const int* weights, int r) {
  int result = 0;
  for (int i = 0; i < r; i++) result += (int)((long)vals[i] * weights[i] % MOD);
  return result % MOD;
}
...
void lagrange_recover(const int* xs, const int* ys, int r, int* coeffs_out) {
  ...
  for (int i = 0; i < r; i++) {
    ...
    weights[i] = (int)((long)num * inv_table[den] % MOD);
  }
  ...
  for (int k = 0; k < r; k++) {
    coeffs_out[k] = lagrange_eval_at_zero(vals, weights, r);
    for (int i = 0; i < r; i++)
      vals[i] =
